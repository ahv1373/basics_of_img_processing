...
import cv2
import time
from coco_classes import COCOUtils
...
class TorchObjectDetection:
    ...
    def __detect__(self, preprocessed_img: torch.FloatTensor) -> Tuple[List[list], List[int], List[float]]:
        ...
        for i in range(0, len(detections["boxes"])):
            ...
            if confidence > self.confidence_threshold:
                ...
                box = detections["boxes"][i].detach().cpu().numpy()  # We use detach cpu numpy because we are using
                # GPU and we need to move the data to the CPU
    ...
    def __preprocess_frame__(self, frame: np.array) -> torch.FloatTensor:
        ...
        image = transforms.Compose([transforms.ToPILImage(), transforms.ToTensor()])(image)
        # add a batch dimension, i.e., the shape of the input image will be (1, 3, H, W)
...
if __name__ == "__main__":
    ...
    if INFERENCE_ON_IMAGES is True:
        ...
        print(f"Detected {len(img_list)} images in the directory")
        img_list.sort()
        for img_name in img_list:
            ...
            key = cv2.waitKey(0)  # wait for 1ms before moving on to the next frame
            if key == ord("q"):
                break
            elif key == ord("s"):
    ...
    # Try on webcam
    if INFERENCE_ON_WEBCAM is True:
        webcam = cv2.VideoCapture(0)
        average_fps = 0
        while True:
            ret, frame = webcam.read()
            start_time = time.time()
            bboxes_list, class_index_list, scores_list = object_detector.exec(frame)
            ...
            # average fps with only 2 decimal points
